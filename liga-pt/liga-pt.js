...
const CONFIG = {
    // Nome da pasta de cache local (guarda imagens dos logos)
    cacheFolder: "LigaPortugalWidgetAssets",
...
// Cores da equipa por nome em português (primária + secundária, da API da ESPN)
const TEAM_COLORS = {
    "Académico de Viseu": ["#000000", "#C60000"],
...
// Mapa de nomes da ESPN para o nome da equipa em português
// (corresponde ao campo "name" da API da ESPN).
const ESPN_NAME_MAP = {
    "Académico de Viseu": "Académico de Viseu",
    "Alverca": "Alverca",
    "Arouca": "Arouca",
    "Benfica": "Benfica",
    "Braga": "Braga",
...
// Jogos estáticos de reserva (usados se a API da ESPN falhar), por jornada e
// nome da equipa em português.
const STATIC_FIXTURES = [
    ["1", "Benfica", "Santa Clara"], ["1", "FC Porto", "Sporting CP"],
    ["1", "Braga", "Vitória SC"], ["1", "Nacional", "Famalicão"],
    ["2", "Sporting CP", "Benfica"], ["2", "Santa Clara", "FC Porto"],
    ["2", "Vitória SC", "Braga"], ["2", "Famalicão", "Nacional"],
    ["3", "Benfica", "FC Porto"], ["3", "Sporting CP", "Braga"],
    ["3", "Vitória SC", "Santa Clara"], ["3", "Nacional", "Famalicão"],
];
...
try {
    ...
    if (data.live) {
        await renderScoreboardCard(widget, data.live);
    } else {
...
async function chooseDebugData() {
    ...
    if (choice === 0) {
        return { live: makeDemoLive("Benfica", "2", "FC Porto", "1", "75'"), matches: [] };
    }
    if (choice === 1) {
        return { live: makeDemoLive("Sporting CP", "1", "Braga", "1", "60'"), matches: [] };
    }
    if (choice === 2) {
        return { live: makeDemoLive("Santa Clara", "0", "Vitória SC", "1", "82'"), matches: [] };
    }
    if (choice === 3) {
        return {
            live: null,
            matches: [
                { homeTeam: "Benfica", awayTeam: "Santa Clara", day: "Hoje", dateText: monthDayLabel(new Date()), time: "18:00", state: "post", homeScore: "2", awayScore: "0", homeLogo: null, awayLogo: null },
                { homeTeam: "FC Porto", awayTeam: "Sporting CP", day: "Hoje", dateText: monthDayLabel(new Date()), time: "20:30", state: "post", homeScore: "1", awayScore: "1", homeLogo: null, awayLogo: null },
                { homeTeam: "Braga", awayTeam: "Famalicão", day: "Hoje", dateText: monthDayLabel(new Date()), time: "21:00", state: "pre", homeLogo: null, awayLogo: null },
                { homeTeam: "Vitória SC", awayTeam: "Gil Vicente", day: "Hoje", dateText: monthDayLabel(new Date()), time: "21:00", state: "pre", homeLogo: null, awayLogo: null },
            ],
        };
    }
    if (choice === 4) {
...
async function fetchMatchData() {
    try {
        ...
        const url = "https://site.api.espn.com/apis/site/v2/sports/soccer/por.1/scoreboard?dates="
...
// Fix 1: todayMatches usa m.day === "Hoje"
// Fix 4: depois de todos os jogos de hoje terminarem, mantém
//        os resultados por 180 minutos antes de mudar para próximos jogos.
// ─────────────────────────────────────────────
...
async function renderScoreboardCard(w, match) {
    try {
    ...
    await addHeroAvatar(row, match.homeTeam, homeEmotion, "home", match.homeLogo);
    row.addSpacer(UI.avatarScoreGap);
    addScoreBlock(row, match);
    row.addSpacer(UI.avatarScoreGap);
    await addHeroAvatar(row, match.awayTeam, awayEmotion, "away", match.awayLogo);
}
...
async function addHeroAvatar(row, teamName, emotion, side, logoUrl) {
    ...
    imageRow.addSpacer();
    const image = await loadAvatar(teamName, logoUrl);
    if (image) {
        ...
        if (colors) {
            const img = imageRow.addImage(makeColorDots(colors[0], colors[1]));
            img.imageSize = new Size(40, 20);
        } else {
            const fallback = imageRow.addText("⚽");
            fallback.font = Font.systemFont(64);
        }
    }
    imageRow.addSpacer();
...
function makeColorDots(color1, color2) {
    const w = 22, h = 10, r = 4;
    const ctx = new DrawContext();
    ctx.size = new Size(w, h);
    ctx.opaque = false;
    ctx.respectScreenScale = true;
    ctx.setFillColor(new Color(color1));
    ctx.fillEllipse(new Rect(0, (h - r * 2) / 2, r * 2, r * 2));
    ctx.setFillColor(new Color(color2));
    ctx.fillEllipse(new Rect(w - r * 2, (h - r * 2) / 2, r * 2, r * 2));
    return ctx.getImage();
}
...
async function loadAvatar(teamName, logoUrl) {
    if (!logoUrl) return null;

    const fm = FileManager.local();
    const cacheDir = getCacheDir(fm);
    const fileName = teamName + "_logo.png";
    ...
    const image = await new Request(logoUrl).loadImage();
...
function addScheduleHeader(w, match, firstTime, lastTime) {
    ...
    header.addSpacer();
    const brand = header.addText("🏆 Liga 🇵🇹");
...
function addScheduleFlag(stack, name) {
    const colors = TEAM_COLORS[name];
    if (colors) {
        const img = stack.addImage(makeColorDots(colors[0], colors[1]));
        img.imageSize = new Size(12, 6);
        stack.addSpacer(3);
    }
}
...
function renderError(w, error) {
    ...
    addScheduleHeader(w, { day: "Liga", dateText: monthDayLabel(new Date()) }, "", "");
    w.addSpacer(14);
